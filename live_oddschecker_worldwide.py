...
PREMIUM_COOLDOWN_SECONDS = 1800
PREMIUM_DEBUG_DUMP_INTERVAL_SECONDS = 3600


class PremiumLimitReached(RuntimeError):
    """Oddschecker has replaced the odds grid with its free-view paywall."""


premium_retry_after_monotonic = 0.0
last_premium_debug_dump_monotonic = 0.0
...
def oddschecker_premium_limit_detected(text):
    body_lower = clean(text).lower()

    markers = (
        "you've reached your daily limit for free views",
        "you have reached your daily limit for free views",
        "subscribe to our essential package",
        "unlock unlimited comparison grids",
        "unlimited comparison grids",
        "purchase essential",
    )
...
def premium_cooldown_remaining_seconds():
    return max(
        0.0,
        premium_retry_after_monotonic
        - time.monotonic(),
    )


def premium_cooldown_active():
    return (
        premium_cooldown_remaining_seconds()
        > 0
    )


def activate_premium_cooldown(
    cooldown_seconds=PREMIUM_COOLDOWN_SECONDS,
):
    global premium_retry_after_monotonic
    ...
    print()
    print("=" * 70)
    print("ODDSCHECKER FREE-VIEW LIMIT REACHED")
    print("=" * 70)
    print(
        "Odds comparison grids are unavailable. "
        "Stopping this Oddschecker scan."
    )
    print(
        "Other Pulse live-engine jobs can continue normally."
    )
    print(
        f"Oddschecker cooldown: {cooldown_seconds // 60} minutes"
    )
    print(
        "Next Oddschecker retry: "
        f"{retry_at.isoformat(timespec='seconds')}"
    )
    print("=" * 70)


def maybe_save_premium_debug_artifacts(
    page: Page,
    url: str,
):
    global last_premium_debug_dump_monotonic

    now_monotonic = time.monotonic()

    if (
        last_premium_debug_dump_monotonic
        and (
            now_monotonic
            - last_premium_debug_dump_monotonic
        )
        < PREMIUM_DEBUG_DUMP_INTERVAL_SECONDS
    ):
        return False

    save_oddschecker_debug_artifacts(
        page=page,
        url=url,
...
def save_oddschecker_debug_artifacts(page: Page, url: str, reason: str):
    debug_dir = Path("debug/oddschecker")
    debug_dir.mkdir(parents=True, exist_ok=True)

    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S_%f")
    safe_reason = re.sub(r"[^A-Za-z0-9_-]+", "_", reason)[:80].strip("_") or "failure"
    ...
    try:
        ...
        print(f"Failed to save Oddschecker debug HTML: {exc}")

    try:
        page.screenshot(path=str(png_path), full_page=True)
    except Exception as exc:
        print(f"Failed to save Oddschecker debug screenshot: {exc}")
    ...
    try:
        ...
        print(f"Failed to save Oddschecker debug metadata: {exc}")

    print(f"Saved Oddschecker debug artifacts: {stem}")


def restart_oddschecker_browser(browser_manager, headless=None):
    print("Restarting shared Oddschecker browser session...")

    try:
        browser_manager.stop()
    except Exception as exc:
        print(f"Browser stop warning: {exc}")

    time.sleep(2)
    browser_manager.start(headless=headless)
    print("Shared Oddschecker browser session restarted.")
...
def extract_dom_market_prices(
    ...
    dom_result = page.evaluate(
        ...
        () => {
            ...
            /*
             * A valid runner row may temporarily have only one visible
             * bookmaker price. Keep it: data-bk has already restricted
             * candidates to genuine bookmaker cells below the headers.
             */
...
def apply_dom_prices_to_snapshot(
    ...
    # DOM prices are authoritative. Remove all text-parser fallback
    # bookmaker assignments before attempting alignment so incomplete
    # DOM extraction can never masquerade as genuine bookmaker data.
    for runner in runners:
        runner["prices"] = []
        runner["bookmaker_count"] = 0
        runner["odds_list"] = []
        runner["best_bookmaker"] = None
        runner["best_odds"] = None
        runner["best_odds_decimal"] = None
        runner["worst_bookmaker"] = None
        runner["worst_odds"] = None
        runner["worst_odds_decimal"] = None
        runner["average_odds_decimal"] = None

    if not dom_rows:
        snapshot["dom_alignment_valid"] = False
        print(
    ...
    if len(dom_rows) != len(runners):
        ...
        print(
            ...
            "DOM prices rejected for this snapshot"
        )

        return snapshot

    snapshot["dom_alignment_valid"] = True
...
def race_url_is_current(
    ...
    """
    Return True when a race should still be monitored.

    UK and Irish race URLs are checked against Europe/London time.
    International race URLs are allowed through because the URL alone
    does not expose a reliable meeting timezone. Finished international
    markets are rejected later when no valid odds grid is available.
    """
...
def discover_race_urls(
    ...
    try:
        ...
        try:
            ...
            print(
                "Timed out waiting for Oddschecker race links."
            )
        ...
        # ---------------------------------------
        # Collect EVERY horse racing winner page.
        # ---------------------------------------
        ...
        print(
            f"Past races skipped: {skipped_past}"
        )

        print(
            f"Countdown races skipped: {skipped_countdown}"
        )

        print(
            f"Usable worldwide races: {len(urls)}"
        )
...
def collect_oddschecker_race_with_page(
    ...
    if oddschecker_premium_limit_detected(
        text
    ):
        maybe_save_premium_debug_artifacts(
            page=page,
            url=url,
        )

        raise PremiumLimitReached(
            "Oddschecker free-view limit reached."
        )
    ...
    if not snapshot.get("dom_alignment_valid"):
        raise RuntimeError(
            "DOM runner-price alignment incomplete."
        )

    if runners_with_prices != runner_count:
        raise RuntimeError(
            "Not every runner has genuine DOM bookmaker prices."
        )
...
def collect_all_discovered_races(
    ...
    if premium_cooldown_active():
        remaining = premium_cooldown_remaining_seconds()
    ...
    # ---------------------------------------
    # Collect every successful snapshot here.
    # ---------------------------------------
    ...
    for index, url in enumerate(
        ...
        for attempt in range(1, 4):
            ...
            except PremiumLimitReached as exc:
                last_error = exc
                premium_limited = True

                print(
                    f"Premium limit detected | "
                    f"{url} | {exc}"
                )

                activate_premium_cooldown()
                break
        ...
        if premium_limited:
            print(
                "Remaining Oddschecker races skipped "
                "because the free-view limit is active."
            )
            break
    ...
    if premium_limited:
        print(
            "Oddschecker scan ended early due to "
            "the free-view limit."
        )

    # --------------------------------------------------
    # Build the Horse Arbitrage dashboard once per scan.
    # --------------------------------------------------

    if collected_snapshots:
        try:
            result = engine.run(
            ...
            print(
                "Horse Arb dashboard update failed | "
                f"{exc}"
            )
    ...
    try:
        while True:
            ...
            if (
                ...
                and result.get("saved", 0) == 0
                and not result.get("browser_restarted")
                and not result.get("premium_limited")
            ):
                print(
                    "Entire Oddschecker scan failed; "
                    "restarting browser before next scan."
                )
                restart_oddschecker_browser(
                    browser_manager,
                    headless=headless,
            ...
            print(
                ...
                f"Premium limited: "
                f"{result.get('premium_limited', False)} | "
